"""Wanfang Data source with Vision AI support."""
...
class WanfangSource(BrowserSource):
    """Wanfang Data source with Vision AI automation."""
    ...
    async def _find_results_with_vision(
        ...
        prompt = f"""这是万方数据(Wanfang)学术论文搜索结果页面。搜索关键词是"{query}"。
...
如果看不到论文结果，返回 []"""
    ...
    def _extract_title(self, item) -> str:
        selectors = [".title a", "h3 a", "a.title", ".article-title a", "a[href]"]
        for selector in selectors:
